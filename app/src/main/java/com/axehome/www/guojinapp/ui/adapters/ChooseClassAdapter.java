...
public class ChooseClassAdapter extends BaseAdapter {
    ...
    private int type = 1;
    public ChooseClassAdapter(Activity context, List<BusinessBean> mList,int type) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(type==1){
            holder.tvText.setText(mList.get(position).getOne_class());
        }else if(type==2){
            holder.tvText.setText(mList.get(position).getTwo_class());
        }else if(type==3){
            holder.tvText.setText(mList.get(position).getThree_class());
        }
