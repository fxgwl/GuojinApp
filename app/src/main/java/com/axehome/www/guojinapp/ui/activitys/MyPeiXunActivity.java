...
import com.axehome.www.guojinapp.ui.adapters.UserCourseAdapter;
...
public class MyPeiXunActivity extends BaseActivity {
    ...
    private void initView() {
        userCourseAdapter = new UserCourseAdapter(getApplicationContext(),userCourseBeanList);
        mlvList.setAdapter(userCourseAdapter);
    }
...
//                mPresenter.getDetails(String.valueOf(pageNo));
    ...
    private void getMyCouponList(){
        ...
        OkHttpUtils.post()
                .url(NetConfig.getMyUsercourseList)
                .params(map)
                .build()
                .execute(new StringCallback() {
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        if (jsonObject.getInteger("code") == 0) {
                            if (curPage == 1) {
                                userCourseBeanList.clear();
                            }

                            List<UserCourseBean> courseBeans = new ArrayList<>();
                            courseBeans.addAll(JSON.parseArray(jsonObject.getJSONObject("data").getString("list"), UserCourseBean.class));
                            userCourseBeanList.addAll(courseBeans);
                            if(userCourseBeanList==null || userCourseBeanList.size()==0){
                                Toast.makeText(getApplicationContext(), "你还没有购买课程", Toast.LENGTH_SHORT).show();
                            }
